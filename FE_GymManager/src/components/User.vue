<template>
<div>
    <SupplierAdd v-if="isShowAdd" @toggle-list="toggleList()" :check="5" @reload-page="reloadAdd()"/>
    <SupplierEdit v-if="isShowEdit" :supplierItem="supplierChose"  @toggle-list="toggleList()"  :check="5"  @reload-update-user="reloadUpdate()" />
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        <tr v-for="(item,index) of supplierList" :key="item.id">
                            <td>{{(item.role)?item.role.name :'Chưa Có'}}</td>
                            <td>{{item.name}}</td>
                            <td>{{item.email}}</td>
                            <td>{{item.phone}}</td>
                            <td>{{(item.department)?item.department.name :'Chưa Có'}}</td>
                            <td>
...
export default {
    computed: {
        ...mapState({
            supplierList :state =>state.user.supplierList,
    ...
    methods: {
        ...mapActions('user', ['getSupplier','deleteSupplier','getByPage']),
        ...mapMutations('user',['findByName']),
        changIsShowAdd() {
